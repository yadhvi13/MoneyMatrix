    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {insights.map((insight, index) => (
          <GlassCard
            key={index}
            className="flex flex-col justify-between h-64 group hover:scale-[1.02] transition-all duration-300"
          >
            <div>
              <div className={`p-3 w-fit rounded-xl ${insight.bg} ${insight.color} mb-4`}>
                <insight.icon className="w-6 h-6" />
            ...
            <div className="flex items-end justify-between mt-4">
              <p className={`text-2xl font-black ${insight.color}`}>
                {insight.value}
              </p>

              <button className="p-2 text-black/20 dark:text-white/20 hover:text-blue-500 transition-all duration-300 group-hover:translate-x-1">
                <ArrowRight className="w-5 h-5" />
              </button>
            </div>
          </GlassCard>
      ...
      {/* MAIN INSIGHT CARD */}
      <GlassCard className="p-8 hover:shadow-xl transition-all duration-300" hover={false}>
        <div className="flex items-start gap-6">

          {/* ICON */}
          <div className="p-4 bg-amber-500/20 rounded-2xl animate-pulse">
            <Lightbulb className="w-8 h-8 text-amber-500" />
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-black/60 dark:text-white/60 max-w-2xl">
              Based on your current spending velocity, you are projected to save 
              <span className="text-black dark:text-white font-bold"> {formatCurrency(summary.totalBalance * 1.1)} </span> 
              by the end of next month. This is a significant improvement over your previous quarter's average.
            </p>
            ...
            <div className="flex gap-4">
              ...
              >
                ...
                <p className="font-bold text-black dark:text-white">
                  {formatCurrency(summary.totalBalance * 0.85)}
                </p>
              </div>
              ...
              >
                ...
                <p className="font-bold text-emerald-500">
                  {formatCurrency(summary.totalBalance * 1.1)}
                </p>
              </div>
            </div>
